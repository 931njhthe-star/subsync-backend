...
> 최종 수정: 2026-09-04 (v1.2 — 프로젝트 개요, 계층·명명 규칙, 에이전트 작업 절차,
> 금지사항, 결과 보고 형식 보강)
> 이 문서를 수정하면 상단의 날짜와 버전을 함께 갱신한다.

이 문서는 SubSync 백엔드를 작업하는 모든 사람과 AI 에이전트가 공유하는 운영 매뉴얼이자
작업 계약이다. 사람에게 `README.md`가 프로젝트 사용 안내라면, 이 문서는 AI가 어떤
구조와 절차로 코드를 읽고 수정할지 정의한다. 담당자가 달라도 동일한 API 계약, 데이터
모델, 보안 수준, 검증 결과를 만들 수 있도록 다음 원칙을 지킨다.

- 사용자 요청과 상위 실행 규칙을 우선하고, 이 문서와 `docs/`의 프로젝트 계약을 함께
  확인한다.
- 구현 전에는 기존 코드·문서·테스트를 읽고 중복 기능을 검색한다.
- 구현 후에는 관련 테스트와 문서를 같은 작업 단위에서 갱신하고, 실제로 실행한 검증만
  완료 보고에 기록한다.
- 이 문서의 폴더 구조와 API 상태는 현재 소스의 사실과 일치해야 한다. 설계 문서에만
  있는 예정 기능이나 폴더를 이미 구현된 것으로 간주하지 않는다.

## 1. 프로젝트 기준

### 프로젝트 한눈에 보기

- 제품: YouTube 영어 영상의 이중 자막과 AI Video Tutor를 제공하는 **SubSync**
- 백엔드 역할: Chrome Extension과 Streamlit 대시보드에 JSON REST API 제공, 사용자 학습
  데이터·단어장·시청 이력·Tutor 대화·행동 로그 관리
- 핵심 기술: Python 3.11, FastAPI, `uv`, Supabase PostgreSQL/Auth, Redis, Gemini/Groq
- 기본 실행 모드: 외부 API 키가 없어도 `stub` provider로 Tutor 계약과 테스트를 확인할 수
  있어야 한다.
- API 기본 경로: `/api/v1` (`/health` 제외)

README의 “확장 예정 구조”와 아키텍처 문서는 목표 설계를 설명할 수 있다. 에이전트는
새 파일이나 API를 만들 때 반드시 실제 `app/`, `tests/`, 라우터 등록 상태를 함께 확인하고,
구현되지 않은 항목은 문서에서 `PROPOSED` 또는 `DEPENDENCY`로 유지한다.
...
새 기능은 다음 흐름을 기본으로 한다.

```text
HTTP 요청
  → Router(입력·dependency·상태 코드)
  → Schema(Pydantic 검증)
  → Service 또는 AI 오케스트레이터(비즈니스 규칙)
  → Repository / Cache / 외부 Provider(데이터·외부 I/O)
  → Response Schema
```

- Router는 요청을 해석하고 dependency를 주입하며 결과를 HTTP 응답으로 변환하는 역할만
  한다. Supabase/Redis/LLM 호출과 도메인 조건문을 직접 작성하지 않는다.
- Service는 재사용 가능한 업무 규칙을 담당하고, Repository는 데이터 저장·조회,
  Cache는 캐시 정책, `app/ai/`는 Tutor 문맥·프롬프트·provider 선택을 담당한다.
- `app/main.py`에는 앱 생성, middleware/CORS 설정, 라우터 등록만 둔다. 기능별 로직을
  `main.py`에 추가하지 않는다.
- 현재 해당 계층이 비어 있으면 기존 계약을 깨는 임시 구현을 만들지 말고, 주입 가능한
  stub/fake 또는 최소 adapter로 연결한 뒤 임시 상태를 문서와 테스트에 표시한다.
...
- Python 패키지·모듈·함수·변수는 `snake_case`, 클래스와 Pydantic 모델은 `PascalCase`,
  상수는 `UPPER_SNAKE_CASE`를 사용한다.
- 요청·응답 DTO는 `*Request`, `*Response`처럼 역할이 드러나는 이름을 사용하고,
  공개 서비스 메서드는 동작이 드러나는 동사(`get_`, `create_`, `update_`, `delete_`,
  `build_`, `generate_`)로 시작한다.
- 불리언은 `is_`, `has_`, `can_`, `enable_` 등 상태가 드러나는 이름을 사용한다.
  프로젝트 API 계약에 이미 정의된 `user_id`, `video_id`, `timestamp`, `message_id` 같은
  필드명은 임의로 바꾸지 않는다.
- API 경로는 소문자·슬래시 기반의 기존 REST 규칙을 따르고, 기존 경로·메서드·필드의
  의미를 바꾸지 않는다. 호환성이 깨지면 5절의 API 버전 규칙을 적용한다.
- 짧고 하나의 책임을 가진 함수를 우선한다. “30줄 이하” 같은 숫자를 기계적으로 맞추기
  위해 로직을 쪼개지 말고, 읽기 쉬운 경계와 재사용성을 기준으로 분리한다.

### 금지사항

- `print()`로 운영 로그를 남기거나 토큰·API 키·비밀번호·개인정보를 소스, fixture,
  문서, 로그, 응답에 기록하지 않는다. 구조화된 logger와 마스킹된 식별자를 사용한다.
- 설정값·비밀값을 코드에 하드코딩하지 않는다. 환경변수는 `app/core/config.py`를 통해
  읽고, 공유 파일에는 키 이름과 안전한 빈 예시만 둔다.
- Router에서 Supabase/Redis/LLM을 직접 호출하거나, Service·Repository를 우회하는
  중복 비즈니스 로직을 만들지 않는다.
- 애플리케이션 코드에 임의의 SQL 문자열을 흩뿌리지 않는다. DB 구조 변경은 6절의
  migration SQL로 남기고, 데이터 접근은 기존 repository/client 패턴을 따른다.
- 실 provider·실 DB·실 외부 네트워크에 의존하는 기본 테스트를 만들지 않는다. 필요한
  외부 동작은 fake/stub, fixture, dependency override로 재현한다.
- 요청 body/query의 `user_id`를 신뢰하거나 인증을 우회하지 않는다. 사용자 식별은
  검증된 JWT의 `sub`만 사용한다.
- 요청받지 않은 대규모 리팩터링, 무관한 포맷 변경, 패키지 교체, migration 재작성,
  `git push --force`를 하지 않는다. 커밋·push도 사용자가 명시적으로 요청한 경우에만
  수행한다.
...
모든 구현·문서 변경은 아래 순서를 따른다. 단순 질의나 상태 확인처럼 변경이 없는
작업에는 필요한 단계만 적용한다.

1. **요청과 범위 파악**: 변경 대상, 성공 조건, 영향받는 담당 영역과 공유 파일을
   먼저 식별한다. 요구사항이 명세와 충돌하거나 결과를 크게 바꾸는 선택지가 있으면
   추측으로 진행하지 않고 확인을 요청한다.
2. **현황 조사**: `rg`/`rg --files`로 관련 구현·스키마·라우터·테스트·문서를 찾고,
   같은 기능이나 기존 패턴이 있는지 확인한다. 설계 문서만 보고 새 구조를 가정하지
   않는다.
3. **계약 확인**: API 변경이면 공통/도메인 API 명세, DB 변경이면 스키마와 migration,
   Tutor 변경이면 AI 설계와 provider/fallback 규칙을 먼저 읽는다.
4. **최소 변경 구현**: 기존 계층·네이밍·응답 형식을 유지하고, 필요한 파일만 수정한다.
   API·DB·환경변수 계약을 바꾸면 같은 작업에서 관련 DTO, 문서, Postman, 테스트를
   함께 갱신한다.
5. **실패 경로 확인**: 성공 경로뿐 아니라 입력 오류, 인증 누락·실패, 소유권 오류,
   외부 provider/cache 실패 등 해당 기능의 대표 실패 경로를 검증한다.
6. **검증 실행**: 변경 위험도에 맞춰 `uv run pytest`, 필요한 직접 API 확인,
   `git diff --check`를 실행한다. 실행하지 못한 명령과 이유는 숨기지 않는다.
7. **변경 범위 점검**: `git status --short`, `git diff --stat`, `git diff`로 의도하지
   않은 파일·비밀값·생성물을 확인한다. 기존 사용자 변경은 되돌리거나 덮어쓰지 않는다.
8. **결과 보고**: 아래 응답 형식으로 변경 파일, 검증 결과, 미검증 항목과 후속 조치를
   짧고 구체적으로 남긴다.

### 에이전트 결과 보고 형식

변경 작업의 최종 응답은 다음 순서를 기본으로 한다.

1. **결과 요약**: 무엇이 완료되었는지 한두 문장으로 먼저 말한다.
2. **변경 파일**: 파일별 핵심 변경과 API/DB/환경변수 계약 변경 여부를 적는다.
3. **검증 결과**: 실행한 명령과 성공·실패 결과를 적는다. 예: `uv run pytest`,
   `git diff --check`.
4. **주의사항/후속 작업**: 테스트하지 못한 외부 연동, 수동 migration 적용, 담당자 리뷰,
   남은 `PROPOSED` 항목이 있으면 명시한다.

코드 자체를 요청받은 경우에도 설명만 길게 반복하지 말고, 실제 변경 결과와 검증 근거를
중심으로 보고한다. 변경이 없었다면 “변경 없음”과 조사 결과를 분명히 구분한다.

### 사용자 확인이 필요한 경우

다음 상황에서는 안전한 읽기·분석을 먼저 끝낸 뒤 작업을 멈추고 확인을 요청한다.

- 운영 데이터 삭제·대량 갱신·`DROP`, 이미 적용된 migration 수정, 강제 push 등 복구가
  어려운 작업
- API/DB 계약을 깨는 변경인데 v2 분리 여부가 정해지지 않은 경우
- 명세와 요청이 충돌하여 어느 쪽을 택해도 결과가 달라지는 경우
- 다른 담당 영역의 도메인 규칙이나 공유 파일을 임의로 설계해야만 완료되는 경우

단순한 파일명·구현 위치처럼 기존 패턴으로 안전하게 결정할 수 있는 사항은 합리적인
가정을 명시하고 진행한다.
...
- [ ] 코드가 책임 경계에 맞게 배치되어 있다. 경계가 애매했다면 10절에 따라
  처리했다.
- [ ] 프로젝트의 실제 폴더·라우터·테스트 상태와 문서의 `IMPLEMENTED`/`PROPOSED`/
  `DEPENDENCY` 표기가 일치한다.
- [ ] 공개 함수·클래스·라우터에 초심자도 이해할 수 있는 docstring이 있고, 복잡한
  로직에는 "왜"를 설명하는 최신 주석이 있다.
- [ ] 모듈·함수·변수·클래스·DTO 이름이 네이밍 규칙을 따르고 기존 API 필드명을
  임의로 바꾸지 않았다.
...
- [ ] 최종 보고에 결과 요약, 변경 파일, 실행한 검증, 미검증 항목과 후속 작업을
  명시했다.
